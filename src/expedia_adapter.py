"""Expedia-Adapter [CRUX-MK].

Expedia-EQC-API + Rate-Management-API + 20%-Kommission-Tracker.

K12 Provenance: jede Response hat source-tracking-fields.
K13 PAV: Real-Bookings require DF_OTA_EXPEDIA_PHRONESIS_TICKET ENV-Var.
ENV-Var-gated: DF_OTA_EXPEDIA_REAL_ENABLED=false (Default) -> Mock.
...
class ExpediaConnector(OTAAdapter):
    """Expedia EQC-API + Rate-Management-API Connector.

    Sandbox-Default: deterministische Mock-Daten.
    Real-Mode: HTTP-Calls an Expedia EQC-API + Rate-Management-API.
    """

    COMMISSION_PCT = 0.20    # 20% Expedia Standard-Kommission
    VENDOR = "expedia"

    MOCK_HOTELS = {
        "hildesheim": {"expedia_property_id": "exp-mock-hildesheim-001", "rooms": 80},
        "cape-coral": {"expedia_property_id": "exp-mock-cape-coral-001", "rooms": 60},
        "munich": {"expedia_property_id": "exp-mock-munich-001", "rooms": 120},
    }

    def __init__(self, sandbox_mode: Optional[bool] = None):
        self.adapter_name = "expedia-ota"
        if sandbox_mode is None:
            self.sandbox_mode = os.environ.get("DF_OTA_EXPEDIA_REAL_ENABLED", "false") != "true"
        else:
    ...
    def connect(self, credentials: dict) -> bool:
        """Establish EQC-API + Rate-Management-API connection. K11 try/except, LC4 idempotent."""
        try:
            ...
            if not hotelier_id or not api_key:
                logger.warning("[expedia] missing credentials for connect")
                self._connected = False
                return False
            ...
            logger.error(f"[expedia] connect failed: {e}")
            self._connected = False
            return False

    def query_inventory(self, hotel_id: str, date_range: tuple) -> list[dict]:
        """Query Inventory via EQC-API (read-only)."""
        op = "query_inventory"
        try:
            ...
            logger.warning("[expedia] real-api query_inventory not yet implemented")
            return []
        except Exception as e:
            logger.error(f"[expedia] query_inventory failed: {e}")
            return []

    def push_rate(self, hotel_id: str, room_type: str, date_iso: str, rate_eur: float) -> bool:
        """Push neue Rate an Expedia Rate-Management-API.
        ...
        try:
            ...
            ticket = os.environ.get("DF_OTA_EXPEDIA_PHRONESIS_TICKET", "")
            if not ticket:
                logger.warning("[expedia] K17-PAV: missing PHRONESIS_TICKET for push_rate")
                return False

            logger.warning("[expedia] real-api push_rate not yet implemented")
            return False
        except Exception as e:
            logger.error(f"[expedia] push_rate failed: {e}")
            return False
    ...
    def pull_bookings(self, hotel_id: str, since_iso: str) -> list[dict]:
        ...
        try:
            ...
            logger.warning("[expedia] real-api pull_bookings not yet implemented")
            return []
        except Exception as e:
            logger.error(f"[expedia] pull_bookings failed: {e}")
            return []
