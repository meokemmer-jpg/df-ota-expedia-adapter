"""Expedia-Webhook-Handler [CRUX-MK].
...
class ExpediaWebhookHandler:
    ...
    def __init__(self, secret: Optional[str] = None, sandbox_mode: Optional[bool] = None):
        ...
        self.secret = secret or os.environ.get(
            "DF_OTA_EXPEDIA_WEBHOOK_SECRET", "df-ota-expedia-mock-webhook-secret"
        )
        ...
        try:
            ...
            if timestamp is not None:
                ...
                if age > self.REPLAY_WINDOW_S:
                    logger.warning(f"[expedia-webhook] replay window exceeded: {age:.0f}s")
                    return False
            expected = hmac.new(
            ...
            logger.error(f"[expedia-webhook] verify_signature failed: {e}")
            return False

    def parse_event(self, body: bytes) -> dict:
        """Parse Expedia-Notification Body (JSON)."""
        import json
        try:
            return json.loads(body.decode("utf-8"))
        except Exception as e:
            logger.error(f"[expedia-webhook] parse_event failed: {e}")
            return {}
