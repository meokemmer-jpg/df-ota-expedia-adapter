"""Tests fuer Expedia OTA-Adapter [CRUX-MK]. Welle-37."""
...
from src.expedia_adapter import ExpediaConnector, AdapterResponse, OTAAdapter
from src.expedia_auth import ExpediaAuthManager, ExpediaCredentials
from src.expedia_webhook import ExpediaWebhookHandler, WebhookVerificationResult
from src.commission_tracker import CommissionTracker, CommissionRecord
from src.adapter_orchestrator import ExpediaAdapterOrchestrator, LoopReport
from src.audit_logger import AuditLogger, AuditEntry


class TestExpediaAdapter:
    def test_sandbox_default(self):
        c = ExpediaConnector()
        assert c.sandbox_mode is True
        assert c.adapter_name == "expedia-ota"
        assert c.COMMISSION_PCT == 0.20

    def test_connect_sandbox(self):
        c = ExpediaConnector(sandbox_mode=True)
    ...
    def test_push_rate_real_no_phronesis(self, monkeypatch):
        monkeypatch.setenv("DF_OTA_EXPEDIA_REAL_ENABLED", "true")
        monkeypatch.delenv("DF_OTA_EXPEDIA_PHRONESIS_TICKET", raising=False)
        c = ExpediaConnector(sandbox_mode=False)
        c._connected = True
    ...
    def test_get_capabilities(self):
        ...
        assert caps["vendor"] == "expedia"
        assert caps["commission_pct"] == 0.20
        assert caps["feature_flags"]["eqc_api"] is True
...
class TestExpediaAuth:
    def test_sandbox_credentials(self):
        a = ExpediaAuthManager(sandbox_mode=True)
    ...
    def test_real_mode_no_env(self, monkeypatch):
        monkeypatch.setenv("DF_OTA_EXPEDIA_REAL_ENABLED", "true")
        monkeypatch.delenv("EXPEDIA_HOTELIER_ID", raising=False)
        monkeypatch.delenv("EXPEDIA_API_KEY", raising=False)
        a = ExpediaAuthManager()
        assert a.get_credentials("hildesheim") is None


class TestExpediaWebhook:
    def test_verify_valid_signature(self):
        h = ExpediaWebhookHandler(secret="test-secret", sandbox_mode=True)
        body = b'{"event_type":"booking_created","booking_id":"exp-123"}'
        import hmac, hashlib
    ...
    def test_handle_webhook_valid(self):
        h = ExpediaWebhookHandler(secret="test-secret", sandbox_mode=True)
        body = b'{"event_type":"booking_created","booking_id":"exp-456"}'
        import hmac, hashlib
        sig = hmac.new(b"test-secret", body, hashlib.sha256).hexdigest()
        result = h.handle_webhook(body, sig)
        assert result.valid is True
        assert result.event_type == "booking_created"
        assert result.booking_id == "exp-456"
...
class TestCommissionTracker:
    def test_record_booking(self, tmp_path):
        ...
        rec = t.record_booking({
            "booking_id": "exp-1", "hotel_id": "hildesheim", "rate_eur": 100.0,
        })
        assert rec.commission_eur == 20.0
        assert rec.commission_pct == 0.20
    ...
    def test_aggregate_period_with_records(self, tmp_path):
        ...
        assert agg["total_commission_eur"] == 60.0  # 3 * 20
...
class TestOrchestrator:
    def test_orchestrator_init(self):
        o = ExpediaAdapterOrchestrator()
        assert o.DF_ID == "df-ota-expedia-adapter"
    ...
    def test_orchestrator_run_sandbox(self, tmp_path, monkeypatch):
        ...
        assert report.df_id == "df-ota-expedia-adapter"
